...
```mermaid
sequenceDiagram
    participant C as 🖥️ New Device
    participant S as 🏷️ DHCP Server
    C->>S: 1️⃣ Discover ("Is there a DHCP server out there?")
    S->>C: 2️⃣ Offer ("Yes — here's an available address")
    C->>S: 3️⃣ Request ("I'd like to use that address")
    S->>C: 4️⃣ Acknowledge ("Confirmed — it's yours")
```
...
```mermaid
flowchart LR
    D["🔍 Discover"]:::warn --> O["📩 Offer"]:::info --> R["✅ Request"]:::warn --> A["🎉 Acknowledge"]:::good
...
```mermaid
flowchart TB
    L["💻 Your Laptop<br/>(new to this network)"]:::info --> Process["🏷️ DORA Process"]:::warn --> Ready["✅ Fully configured:<br/>IP, subnet mask, gateway"]:::good
